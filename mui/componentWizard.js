...
import {
    ...
    CardContent, Checkbox, CircularProgress, DialogTitle, FormControl, FormControlLabel,
    FormHelperText, Grid, IconButton, InputAdornment, InputLabel, ListItemText, MenuItem, Select, Switch, TextField, ToggleButton, ToggleButtonGroup, Tooltip, Typography
} from '@mui/material';
    ...
    return (
        <Grid item xs={xs} sm={sm} md={md} lg={lg} xl={xl} key={label}>
            ...
            >
                ...
                >
                    {options && options.map((el) => <MenuItem key={el.value} value={el.value}>{el.name}</MenuItem>)}
                </Select>
...
dialog.multiSelect = (props) => <Field {...props} as={d_ms} />
const d_ms = ({ label, xs, sm, md, lg, xl, form, required, options, value, setFields, renderValue, callback, ...props }) => {
    const { errors, touched, handleChange, setFieldValue } = form;
    const err = _.get(touched, props.name) && _.get(errors, props.name);
        ...
        <Grid item xs={xs} sm={sm} md={md} lg={lg} xl={xl} key={label}>
            ...
            >
                ...
                <Select
                    ...
                    displayEmpty
                    disabled={props.disabled || false}
                    renderValue={selected => options.filter(el => selected.includes(el.value)).map(el => el.name).join(', ')}
                    onChange={(e) => {
                        if (setFields) {
                            setFields.forEach(el => {
                                if (!el['if'] || e.target.value === el['if']) setFieldValue(el['name'], el['value']);
                ...
                >
                    {options && options.map((el) => <MenuItem key={el.value} value={el.value}>
...
filter.dropDownList = ({ filters, setFilters, name, label, minWidth, options }) => (
    <Grid item>
        <FormControl sx={{ minWidth: minWidth }} size="small">
            ...
            >
                ...
                {options[name].map(el => typeof el === 'string' ?
                    <MenuItem key="empty" value={el}>{el}</MenuItem> :
...
filter.checkbox = ({ filters, setFilters, name, label }) => (
    <Grid item>
        <FormControlLabel
            value={filters[name] ?? false}
            control={
                <Checkbox name={name} onChange={(e) => {
                    const newFilter = { ...filters };
...
filter.numberField = ({ filters, setFilters, name, label, width, allowNegative }) => (
    <Grid item>
        <TextField
            label={label}
            id={name}
            ...
            style={{ width: width }}
            onChange={e => {
                if (!isNaN(e.target.value) || !e.target.value || (allowNegative && e.target.value === "-")) {
                    const newFilter = { ...filters };
...
filter.textField = ({ filters, setFilters, name, label, width }) => (
    <Grid item>
        <TextField
            label={label}
            id={name}
...
export { general, dialog, dashboard, filter };
